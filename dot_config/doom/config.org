...
#+BEGIN_SRC emacs-lisp :tangle config.el
(setq doom-font
      (font-spec :family "Iosevka NF" :size 18))
(setq doom-big-font
...
(after! org-superstar
  ...
  (setq org-superstar-headline-bullets-list '("🟍" "✫" "⌘" "⁂" "♁" "υ" "♭" "⚡")
  ...
  (set-face-attribute 'org-level-3 nil :inherit 'outline-3 :height 1.22) ;\large
  (set-face-attribute 'org-level-2 nil :inherit 'outline-2 :height 1.35) ;\Large
  (set-face-attribute 'org-level-1 nil :inherit 'outline-1 :height 1.44) ;\LARGE
...
(use-package! denote
  ...
  (map!
   :leader
   :map global-map
        "f x" #'denote-rename-file
        "f X" #'denote-rename-file-using-front-matter
        ; ("n j") #'my-denote-journal
        "n j" #'my-denote-journal
        :desc "denote" "n d" #'denote
        :desc "denote in subdirectory" "n s" #'denote-subdirectory)
;;  (map!
;;   :leader
;;   :map org-mode-map
...
;;        "n f b" #'denote-link-find-backlink)
  (map!
   ...
   :map dired-mode-map
   (:prefix ("d" . "denote")
        :desc "Link marked notes" "i" #'denote-link-dired-marked-notes
        :desc "Rename marked files" "r" #'denote-dired-rename-marked-files
        :desc "Renamed marked files using frontmatter" "R" #'denote-dired-rename-marked-files-using-front-matter))
)
#+end_src
